...
**VizU** is a tool for visualising uncertainty in spatial data, and this vignette demonstrates how to use **VizU** to create bivariate maps, pixel maps and glyph maps, the three uncertainty visualisation methods developed in Lucchesi & Wikle (2017).

There are several types of functions in **VizU**: formatting functions, building functions and viewing functions. Formatting functions prepare data frames and spatial polygons for use in building functions; building functions create maps, animations, keys and colour palettes; and viewing functions plot these objects. Some of these functions are useful across all three map types, such as `read.uv`, while others are map specific, such as `pixelate`. Below is a table displaying the functions that can be used with each map type. Blue functions format. Green functions create. Red functions plot.

The bivariate and glyph functions can be applied to areal or point level data, while pixel functions are meaningful only for areal data.

```{r echo = FALSE, fig.align='center', fig.height=6, fig.width=6, fig.asp=.5, warning=FALSE}
library(knitr)
library(ggplot2)
...
Two datasets are included in the package. `us_data` contains the 2015 county poverty rates for American families, and `us_geo` is a spatial polygons data frame for a US county map, which can be used to map the estimates and errors in `us_data`. These data come from the US Census Bureau Fact Finder website, and documentation can be found under `?us_data` and `?us_geo`.
...
## Build bivariate maps

A bivariate map is based on a bivariate colour palette, which is created by blending two single hue colour palettes. One colour palette represents the variable of interest while the other represents the uncertainty, and when the palettes are blended, each colour in the new palette becomes simultaneously representative of both a variable class and uncertainty class.

There are four prepared bivariate colour palettes included in **VizU**: 'BlueYellow', 'CyanMagenta', 'BlueRed' and 'GreenBlue'.
...
You can also design your own bivariate colour palette with `build_palette`. Instead of entering one of the four palettes into `name`, you enter `'usr'` and then choose two colours and specify the colour change for each. If you choose two light colours when creating your own palette, the generated palette will lack important differences in colour across the grid. Therefore, it is best to use two darker colours. For example, 'chartreusu4 and 'darkblue' work better than 'tan2' and 'lightskyblue' (see comparison below). The numeric vector passed to `difC` controls the colour change across each single hue colour gradient.
...
```{r, echo = TRUE, fig.align = "center", fig.width=8, fig.height=6, fig.asp=.75}
...
```{r, echo = TRUE, fig.align = "center", fig.width=8, fig.height=9, fig.asp=.5, message = FALSE}
attach_key(m, k)
```
...
If `distribution = "normal"`, the values assigned to pixels will be drawn from a normal distribution parameterised using the estimates and errors (means and standard deviations).
```{r, echo = TRUE, fig.height=8, fig.width=8}
#normal distribution
...
If `distribution = "discrete"`, a data frame of quantiles, which define the relative frequency distributions for the estimates, must be passed to `q`. Below is what the California map would look like if the relative frequencie distributions for the estimates followed exponential distributions.
```{r, echo = TRUE, fig.height=8, fig.width=8}
...
d_m <- build_pmap(data = ca_data, distribution = "discrete", pixelGeo = pix, id = "region", q = ca_data.q)
view(d_m)
```

The map can also be animated with `animate` so that the pixels flicker between a series of assigned values. `view` saves the animation to your computer as an html file and automatically opens a browser to view it. A longer `aniLength` corresponds to a longer animation as well as a longer running time. Generating the animation with `view` can take several minutes.
...
The `build_gmap` function will plot a glyph at each region centroid. The colour of the glyph represents the estimated poverty rate among families, and the rotation of the glyph represents the margin of error for the estimate.
...
Maps and keys can be viewed together with `attach_key`.
```{r, echo = TRUE, fig.align = "center", fig.width=8, fig.height=6, fig.asp=.5, message = FALSE}
attach_key(map, key)
```

You can change the glyph size, shape and colour as well as the added borders.
```{r, echo = TRUE, fig.align = "center", fig.width=8, fig.height=8, fig.asp=.75}
#build a glyph map
