---
title: "Visualising Uncertainty in Poverty Rates across the USA"
...
usBivMap <- build_bmap(data = poverty, shapefile = us_geo, id = "GEO_ID", terciles = TRUE)
view(usBivMap)
```
...
#make some changes
usBivMapDif <- build_bmap(data = poverty, shapefile = us_geo, id = "GEO_ID", terciles = FALSE, palette = customBivPal3)
view(usBivMapDif)
```
...
# build a glyph map
usGlyphMap <- build_gmap(data = co_data, shapefile = co_geo, id = "GEO_ID", size = 80, glyph = "icone", border = "state")
view(usGlyphMap)
```
...
```{r, echo = TRUE, fig.align = "center", fig.width=8, fig.height=8, fig.asp=.75}
# build a glyph map
usGlyphMapDif <- build_gmap(data = co_data, shapefile = co_geo, id = "GEO_ID", size = 70, border = "county", glyph = "semi", palette = "Reds")
view(usGlyphMapDif)
```
...
usExcMap <- build_emap(data = poverty, pdflist = pdflist, shapefile = us_geo, id = "GEO_ID", key_label = "Pr[X > 30]")
view(usExcMap)
```
