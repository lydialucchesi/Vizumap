...
build_emap <-
  function(data,
    ...
    if (!is.null(border)) {
      m <-
        (border %in% c("county", "france", "italy", "nz", "state", "usa", "world"))
      if (length(m) == 0) {
        stop(
          "Border name not recognised. Must be one of county, france, italy, nz, state, usa or world \n
    ...
    if (!(palette %in% c("YlOrBr", "YlOrRd", "YlGnBu", "PuBuGn"))) {
      stop(
        "Palette name not recognised. Must be one of YlOrBr, YlOrRd, YlGnBu or PuBuGn, \n
    ...
    if (!is.null(geoData) &
        inherits(geoData, "SpatialPolygonsDataFrame")) {
      geoData@data <-
        geoData@data %>% dplyr::mutate_if(is.factor, as.character)
    ...
    if (is.null(pdflist)) {
      ...
      col_ids <- match(names(data)[1:3], names(output_data))
      names(output_data)[col_ids] <- c("pr_exc", "estimate", "error")
    } else {
      # assume 2 columns and pr_exc is to be calculated via a function
      warning(
        "Ensure the pdf you select is suitable for your data. See ??build_emap for examples of good and bad distribution choices.\n"
      )

      col_ids <- match(names(data)[1:2], names(output_data))
      names(output_data)[col_ids] <- c("estimate", "error")
      estimate <- output_data$estimate
      error <- output_data$error

      args_call <-
        eval(do.call("substitute", list(pdflist$args, list(estimate, error))))
      output_data$pr_exc <-
        pdist(pname = pdflist$dist,
              th = pdflist$th,
              args = args_call)
